...
struct ContentView: View {
    @ObservedObject var taskList: SimpleTaskList
    @State private var isCompleted: [Bool] = [] // Added to track completion status
    @State private var isAddingTask = false // Added to control the Add Task sheet
    
    init(taskList: SimpleTaskList) {
        self.taskList = taskList
        // Initialize isCompleted array to match the task completion status
        self._isCompleted = State(initialValue: Array(repeating: false, count: taskList.tasks.count))
    }
                ...
                ForEach(taskList.tasks.indices, id: \.self) { index in
                                    HStack {
                                        Button(action: {
                                            ...
                                            taskList.tasks[index].isCompleted = isCompleted[index]
                                        }) {
                                            Image(systemName: isCompleted[index] ? "checkmark.square" : "square")
                                                .imageScale(.large)
                                        }
                        
                        Text(taskList.tasks[index].title)
                            .strikethrough(isCompleted[index], color: .gray)
                            .foregroundColor(isCompleted[index] ? .gray : .primary)
                    }
                }
